    ...
    <div class="widget border-b-0">
        @if (auth()->check())
            @if (auth()->check() && auth()->user()->isAdmin())
                <a class="btn is-green w-full" href="{{ route('admin.channels.index') }}">Add New Channel</a>
                <br><br>
            @endif
            @if(auth()->user()->confirmed)
                <button class="btn is-green w-full" @click="$modal.show('new-thread')">Add New Post</button>
            @else
                <p class="text-xs text-grey-dark font-bold border border-dashed border-grey-dark p-3">Please confirm your email
                    address to participate.</p>
            @endif
        @else
